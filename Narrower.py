...
expected_columns = [
    ...
    'rdfs:isDefinedBy rdf:resource', 'skos:note xml:lang="en".1', 'skos:note xml:lang="en".2', 'skos:narrower'
]

# Load Excel data
excel_file = r'C:\Users\jenni\NI2Automation\Narrower Match Terms.xlsx'
...
for index, row in df.iterrows():
    ...
    if term in unique_terms:
        class_uri = unique_terms[term]
    else:
        class_uri = URIRef(nis2v[term])
        unique_terms[term] = class_uri
    ...
    # Handle subClassOf, skos:note, and skos:narrower, even for duplicates
    ...
    if pd.notna(row['skos:narrower']):
        g.add((class_uri, SKOS.narrower, URIRef(row['skos:narrower'])))

    print(f"Added triples for row {index + 1}")

# Serialize graph to RDF/XML
rdf_file = r'C:\Users\jenni\OneDrive\Documents\UCD Cybersecurity Masters\COMP47830 - Cybersecurity Research Project\Protege\Automation-Output\Narrower_Match_Terms_v1.rdf'
