...
expected_columns = [
    ...
    'rdfs:isDefinedBy rdf:resource', 'skos:note xml:lang="en".1', 'skos:note xml:lang="en".2',
    'skos:related', 'skos:related.1', 'skos:related.2'
]

# Load Excel data
excel_file = r'C:\Users\jenni\NI2Automation\Complex Match Terms.xlsx'
...
for index, row in df.iterrows():
    ...
    if class_uri in processed_uris:
        ...
        if pd.notna(row['skos:related']):
            g.add((class_uri, SKOS.related, URIRef(row['skos:related'])))
        if pd.notna(row['skos:related.1']):
            g.add((class_uri, SKOS.related, URIRef(row['skos:related.1'])))
        if pd.notna(row['skos:related.2']):
            g.add((class_uri, SKOS.related, URIRef(row['skos:related.2'])))
    else:
...
rdf_file = r'C:\Users\jenni\OneDrive\Documents\UCD Cybersecurity Masters\COMP47830 - Cybersecurity Research Project\Protege\Automation-Output\Complex_Match_Terms_v1.rdf'
