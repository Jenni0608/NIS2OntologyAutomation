...
expected_columns = [
    ...
    'rdfs:isDefinedBy rdf:resource', 'skos:note xml:lang="en".1', 'skos:note xml:lang="en".2', 'skos:exactMatch.1'
]

# Load Excel data
excel_file = r'C:\Users\jenni\NI2Automation\Exact Match Terms.xlsx'
...
# Dictionary to keep track of seen terms
seen_terms = {}
...
for index, row in df.iterrows():
    ...
    if class_uri not in seen_terms:
        seen_terms[class_uri] = True
    ...
    if pd.notna(row['skos:exactMatch.1']):
        g.add((class_uri, SKOS.exactMatch, URIRef(row['skos:exactMatch.1'])))

    print(f"Added triples for row {index + 1}")

# Serialize graph to RDF/XML
rdf_file = r'C:\Users\jenni\OneDrive\Documents\UCD Cybersecurity Masters\COMP47830 - Cybersecurity Research Project\Protege\Automation-Output\Exact_Match_Terms_v4.rdf'
